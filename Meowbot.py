...
class SelectiveTranslator:
    ...
    def _init_deepl(self):
        """Initialize DeepL translator with retries and longer timeout."""
        ...
        max_retries = 3
        timeout = 30  # seconds, can be overridden with env DEEPL_TIMEOUT
        timeout_env = os.getenv('DEEPL_TIMEOUT')
        if timeout_env:
            try:
        ...
        for attempt in range(1, max_retries + 1):
            try:
                self.deepl_translator = deepl.Translator(deepl_key, timeout=timeout)
                self.deepl_supported = [lang.code.lower() for lang in self.deepl_translator.get_target_languages()]
                logger.info(f"✅ DeepL initialized with {len(self.deepl_supported)} languages (attempt {attempt})")
                return
            except Exception as e:
                logger.warning(f"DeepL init attempt {attempt}/{max_retries} failed: {e}")
                if attempt == max_retries:
                    logger.error("❌ DeepL initialization failed after all attempts. Falling back to Google.")
                    self.deepl_translator = None

    def _to_deepl_code(self, lang_code):
        """Convert internal codes to DeepL format (uppercase)."""
        mapping = {
    ...
    def get_connection(self):
        ...
        return sqlite3.connect('translations.db', check_same_thread=False)

    def _init_db(self):
        """Create tables if they don't exist."""
        try:
            conn = self.get_connection()
            # Determine database type by checking if it's a psycopg2 connection
    ...
    def _execute_query(self, query, params=None, fetchone=False, fetchall=False):
        """Execute a query on either PostgreSQL or SQLite, adapting placeholders."""
        try:
            conn = self.get_connection()
            is_postgres = isinstance(conn, psycopg2.extensions.connection)
